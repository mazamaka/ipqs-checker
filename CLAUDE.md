...
│   │   ├── profile.py       # Profile - уникальный fingerprint
│   │   └── check.py         # Check - каждая проверка
...
| `/admin/profiles` | Список профилей с фильтрами |
| `/admin/profile/{id}` | Детали профиля + история |
| `/admin/history` | Лента всех проверок |

### Функции
- **Группировка по fingerprint** — автоматически по canvas+webgl+device_id
- **Скрытый мониторинг** — пользователь не знает что его проверка фиксируется
- **Пометка профилей** — можно пометить подозрительные профили
- **Статистика** — fraud score, страны, детекты

## База данных

### Таблицы
- **profiles** — уникальные устройства (fingerprint_hash уникален)
- **checks** — все проверки с полными данными IPQS

### Связи
- Profile 1:N Check (один профиль — много проверок)
- Профиль создаётся автоматически при первой проверке
